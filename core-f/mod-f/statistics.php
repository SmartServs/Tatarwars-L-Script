...
class StatisticsModel extends ModelBase{
  ...
  return $tribeId == 0 ? $this->provider->fetchResultSet( "SELECT  p.id, p.tribe_id, p.player_type, p.is_blocked, p.blocked_time, p.gold_num, p.last_ip, p.name, p.alliance_id, color_name, p.alliance_name, is_active, p.total_people_count, p.villages_count,  floor(TIME_TO_SEC(TIMEDIFF(NOW(), p.last_login_date))/3600) lastLoginFromHours,TIME_TO_SEC(TIMEDIFF(DATE_ADD(registration_date, INTERVAL %s SECOND), NOW())) protection_remain_sec FROM p_players p WHERE  p.player_type!=%s ORDER BY (p.total_people_count*10+p.villages_count) DESC, p.id ASC LIMIT %s,%s", array( $protectionPeriod, PLAYERTYPE_TATAR , $pageIndex * $pageSize, $pageSize ) ) : $this->provider->fetchResultSet( "SELECT p.id,p.tribe_id,TIME_TO_SEC(TIMEDIFF(DATE_ADD(registration_date, INTERVAL %s SECOND), NOW())) protection_remain_sec, p.player_type, color_name, p.is_blocked, p.last_ip, p.blocked_time, p.gold_num, p.name, p.alliance_id, p.alliance_name, p.total_people_count, p.villages_count,  floor(TIME_TO_SEC(TIMEDIFF(NOW(), p.last_login_date))/3600) lastLoginFromHours FROM p_players p WHERE  p.player_type!=%s AND p.tribe_id=%s ORDER BY (p.total_people_count*10+p.villages_count) DESC, p.id ASC LIMIT %s,%s", array( $protectionPeriod,PLAYERTYPE_TATAR , $tribeId, $pageIndex * $pageSize, $pageSize ) );
	}
	public function getPlayerRankById($playerId, $tribeId){
  $GameMetadata = $GLOBALS['GameMetadata'];
  $protectionPeriod = intval( $GameMetadata['player_protection_period'] / $GameMetadata['game_speed'] );
		$row = $this->provider->fetchRow( "SELECT  TIME_TO_SEC(TIMEDIFF(DATE_ADD(p.registration_date, INTERVAL %s SECOND), NOW())) protection_remain_sec,p.id, (p.total_people_count*10+p.villages_count) score FROM p_players p WHERE  p.id=%s AND p.player_type!=%s LIMIT 1", array( $protectionPeriod,$playerId, PLAYERTYPE_TATAR ) );
...
LIMIT %s,%s", array($pageIndex * $pageSize, $pageSize));
  ...
  public function getHeroListCount( )
    ...
    public function getHeroRankById( $playerId )
    {
        ...
        return addslashes( $row['hero_troop_id'] ) == 0 ? 0 : $this->getHeroRank( $row['id'], $row['score'] );
    }
    ...
    public function getPlayersPointsList( $pageIndex, $pageSize, $isDefense )
    {
     $GameMetadata = $GLOBALS['GameMetadata'];
     $protectionPeriod = intval( $GameMetadata['player_protection_period'] / $GameMetadata['game_speed'] );
        return $this->provider->fetchResultSet( "SELECT  TIME_TO_SEC(TIMEDIFF(DATE_ADD(registration_date, INTERVAL %s SECOND), NOW())) protection_remain_sec,p.id, p.name, p.total_people_count, p.villages_count, p.%s points FROM p_players p WHERE  p.player_type!=%s ORDER BY (p.%s) DESC, p.id ASC LIMIT %s,%s", array( $protectionPeriod,$isDefense ? "defense_points" : "attack_points", PLAYERTYPE_TATAR , $isDefense ? "defense_points" : "attack_points", $pageIndex * $pageSize, $pageSize ) );
    }
    public function getPlayersPointsList2( $pageIndex, $pageSize, $isDefense )
    {
        return $this->provider->fetchResultSet( "SELECT  p.id, p.name, p.tribe_id, p.total_people_count, p.villages_count, p.thief_points points FROM p_players p WHERE  p.player_type!=%s ORDER BY (p.thief_points) DESC, p.id ASC LIMIT %s,%s", array(  PLAYERTYPE_TATAR , $pageIndex * $pageSize, $pageSize ) );
    }
    ...
    {
        return $this->provider->fetchResultSet( "SELECT id, medals, %s FROM p_players WHERE  %s > 0 ORDER BY %s DESC, id ASC LIMIT 10", array( $columnName,$columnName, $columnName, $columnName ) );
    }
    public function getTop10( $isPlayer, $columnName )
    { $GameMetadata = $GLOBALS['GameMetadata'];
     $protectionPeriod = intval( $GameMetadata['player_protection_period'] / $GameMetadata['game_speed'] );
     error_log("getTop10 isp=".$isPlayer." pp=".$protectionPeriod);
       if ($isPlayer) {
     //   return $this->provider->fetchResultSet( "SELECT TIME_TO_SEC(TIMEDIFF(DATE_ADD(t.registration_date, INTERVAL %s SECOND), NOW())) protection_remain_sec, t.id, t.name, color_name, t.%s points FROM %s t WHERE name!='التتار' AND t.%s>0 ORDER BY t.%s DESC, t.id ASC LIMIT 10", array( $protectionPeriod, $columnName, $isPlayer ? "p_players" : "p_alliances", $columnName, $columnName ) );
        return $this->provider->fetchResultSet( "SELECT TIME_TO_SEC(TIMEDIFF(DATE_ADD(t.registration_date, INTERVAL %s SECOND), NOW())) protection_remain_sec, t.id, t.name, color_name, t.%s points FROM %s t WHERE name!='التتار' AND t.%s>0 ORDER BY t.%s DESC, t.id ASC LIMIT 10", array( $protectionPeriod, $columnName, $isPlayer ? "p_players" : "p_alliances", $columnName, $columnName ) );
       }
