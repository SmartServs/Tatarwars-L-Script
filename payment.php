...
$this->contentCssClass = 'payment';
 ...
 error_reporting(E_ALL);
			...
			$HttpClient = new HttpClient(); 
				...
				if($this->providerType == 'edfapay'){
					...
					$userData = $HttpClient->getInstance(); 
					 $name = vc_object($userData , 'name');
					$email = vc_object($userData , 'email');
					 
					$tmpString =  $this->generateRandomString(7);
					 
					 $packageId=(int) $this->packageIndex ;
					 $getPackage = $packages[$packageId];
					 
					 
					 $orderNumber=$getPackage['gold'].'-'.$packageId.str_shuffle('0987654321').time();
					 $orderAmount=$getPackage['cost'];
					 
					  $orderDesc="order";
					 
					$clientIpAddress=  $this->getUserIpAddr();
					...
					$edfa_merchant_id="c0621c41-6531-4bc7-a398-5d29e80c440a";
						...
						$dataToHash=$orderNumber.$orderAmount.$orderCurr.$orderDesc.$merchPass;
						
					
						$hash = sha1(md5(strtoupper($dataToHash)));; 
							
					
			   $db =$HttpClient->connectSQL();
                
                $sQl = $db->query("INSERT INTO `trans_orders` (`order_number`, `player_id`, `paid`) VALUES ('" . $orderNumber . "', '" .  $userData->playerId . "', '0' );");
					$getToken = $HttpClient->generateToken();
					
					$redirectredirectLink=$HttpClient->getUrl().'paymentedfaCheck.php?route=wait&token='.$getToken.'&order_id='.$orderNumber ;
					$curl = curl_init();
				...
				CURLOPT_POSTFIELDS => 
						array(
									'action' => 'SALE'
									,'edfa_merchant_id' =>$edfa_merchant_id
									...
									,'payer_address' => 	$tmpString.'-'.$email
									,'payer_country' => 'SA'
									,'payer_city' => 'Riyadh'
									,'payer_zip' => '12221'
									,'payer_email' =>	$tmpString.'-'.$email
									,'payer_phone' => $phone
									,'payer_ip' => $clientIpAddress
									,'term_url_3ds' =>$redirectredirectLink
									,'auth' => 'N'
									,'recurring_init' => 'N'
									,'hash' => $hash),
						));
		...
		curl_close($curl);
		$redirectLink = json_decode($response);
					
				//print'<pre>'; print_R($redirectLink); print'</pre>';
			//	 die();
					
					header("Location: ".$redirectLink->redirect_url);
die();
...
<script type="text/javascript">
  ...
  function random_int($min, $max) {
        if (!is_int($min) || !is_int($max)) {
            throw new Exception('random_int(): $min and $max must be integers');
        }
        if ($min > $max) {
            throw new Exception('random_int(): $min must be less than or equal to $max');
        }
