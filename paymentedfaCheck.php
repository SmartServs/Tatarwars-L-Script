...
class VCPayment extends Paylink
{
	...
	public function waitData(){
		...
		$getToken = vc_get('token');
		$order_id = vc_get('order_id');
		
		 $redirectredirectLink=$this->getUrl().'paymentedfaCheck.php?route=end&token='.$getToken.'&order_id='.$order_id ;

        print'<h3>Please Wait ...</h3>';

        print '<script>';	 
        print 'setTimeout(function() { window.location.href = "'.$redirectredirectLink.'";  }, 3000);'; 
        print '</script>';
    ...
    public function checkPayment()
    {
		...
		$redirectback=$this->getUrl().'plus';
		 
		if( $resultData['paid']==1){
			 ...
			 $db->query( "INSERT INTO p_msgs 
                        ...
                        print sprintf('<i >تمت عملية الشحن بنجاح بقيمة %s ذهب</i>' , $gold);
							print '<script>';	 
							print 'window.location.href = "'.$redirectback.'";'; 
						print '</script>';
